...
import { ArrowLeft, Save, Check, Search, Trash2, Edit3, MessageCircle } from 'lucide-react';
import Link from 'next/link';
import { useRouter, useSearchParams } from 'next/navigation';
import { useGems } from '@/app/lib/hooks';

// --- VIEW 1: THE EDITOR (Single Note) ---
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FDFBF7]">
      ...
      <header className="flex items-center gap-4 p-6 sticky top-0 z-10 bg-[#FDFBF7]/90 backdrop-blur-md">
        <button 
          onClick={() => router.push('/spiritual-gems')} // Go back to list
        ...
        <div>
          <h1 className="font-serif text-xl font-bold text-stone-900">Edit Note</h1>
          <p className="text-xs text-stone-500 font-medium uppercase tracking-wider">Personal Reflection</p>
        </div>
      </header>

      <div className="flex-1 p-6 max-w-lg mx-auto w-full">
        <div className="bg-white border-l-4 border-amber-500 p-5 mb-8 rounded-r-xl shadow-sm">
          <p className="text-xs font-bold uppercase text-amber-600 mb-1 tracking-widest">Reference</p>
          <p className="font-serif text-2xl text-stone-900 font-bold">{initialRef}</p>
        </div>
        ...
        <div className="space-y-4">
          <textarea 
            ...
            className="w-full h-[50vh] p-6 rounded-2xl bg-white border-2 border-stone-200 
            focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 
            text-lg text-black placeholder:text-stone-300 
            outline-none transition-all resize-none shadow-sm leading-relaxed"
            autoFocus
          ></textarea>
...
// --- VIEW 2: THE LIST (All Notes) ---
function GemsList() {
  const { allGems, deleteGem, isLoaded } = useGems();
  const [searchTerm, setSearchTerm] = useState('');
  
  if (!isLoaded) return <div className="p-10 text-center text-stone-400">Loading gems...</div>;

  const gemsArray = Object.entries(allGems).map(([ref, content]) => ({ ref, content }));
  
  const filteredGems = gemsArray.filter(g => 
    ...
    <div className="flex flex-col min-h-screen bg-[#FDFBF7]">
      ...
      <header className="p-6 sticky top-0 z-10 bg-[#FDFBF7]/90 backdrop-blur-md border-b border-stone-100">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="font-serif text-2xl font-bold text-stone-900">Spiritual Gems</h1>
            <p className="text-xs text-stone-500 font-bold uppercase tracking-widest">{gemsArray.length} Notes Saved</p>
          </div>
        </div>
      ...
      <div className="flex-1 p-6">
        {gemsArray.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 opacity-50 text-center">
            ...
            <p className="font-serif text-xl text-stone-600 mb-2 font-bold">No gems yet</p>
            <p className="text-sm text-stone-400 max-w-xs mx-auto">Start reading and tap "Reply" on a verse to add your first note.</p>
            <Link href="/isaiah/1" className="mt-8 px-6 py-3 bg-stone-900 text-white rounded-full font-bold text-sm uppercase tracking-widest hover:bg-black transition-colors">
              Go to Reading
            </Link>
          </div>
        ) : (
          <div className="grid gap-4 max-w-2xl mx-auto pb-20">
            {filteredGems.length === 0 && (
              <p className="text-center text-stone-400 mt-10">No matches found.</p>
            )}
            
            {filteredGems.map((gem) => (
              <div key={gem.ref} className="bg-white p-5 rounded-2xl shadow-sm border border-stone-100 relative hover:border-amber-200 transition-colors">
                
                {/* Header with Title and Delete Button */}
                <div className="flex justify-between items-start mb-3">
                  <Link href={`/spiritual-gems?ref=${encodeURIComponent(gem.ref)}`} className="flex-1">
                ...
                {/* Content Link */}
                <Link href={`/spiritual-gems?ref=${encodeURIComponent(gem.ref)}`} className="block group">
                  <div className="flex items-center gap-2 mb-2">
                     <p className="text-stone-600 font-serif line-clamp-3 text-lg leading-relaxed group-hover:text-stone-900 transition-colors">
                       {gem.content || <span className="italic text-stone-300">Empty note...</span>}
                     </p>
                  </div>
...
function GemsPageRouter() {
  ...
  // If a reference exists in URL (e.g. ?ref=Isaiah 1:1), show Editor.
  // Otherwise show the List.
  if (ref) {
    return <GemsEditor initialRef={decodeURIComponent(ref)} />;
  }
